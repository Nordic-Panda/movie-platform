...
public static class MovieSeeder
{
    public static async Task SeedMovies(AppDbContext db)
    {
        ...
        var currencies = await db.Currencies.ToDictionaryAsync(c => c.Code);
        ...
        {
            throw new InvalidOperationException(
                "Cannot seed movies because no currencies exist. Seed currencies first."
            );
        }
        ...
        var sek = currencies["SEK"];
        var dkk = currencies["DKK"];
        var jpy = currencies["JPY"];
    ...
    {
        var money = MoneyFactory.Create(budget, currency);
